...
app.onError((err, c) => {
    logger.error({err: err.message, path: c.req.path, query: c.req.query()}, "uncaught error"); // Log it securely on the backend
...
async function safeRunSync(): Promise<void> {
    ...
    recordSyncStart();
    try {
        const { tickersProcessed, tickersFailed, tickersSkipped } = await syncMarketData();
        ...
        logger.info(`[sync] Completed at ${getLastRun()!.at.toISOString()} — ${note}`);
    } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        recordSyncRun({ at: new Date(), ok: false, note: msg });
        logger.error(`[sync] Run failed at ${getLastRun()!.at.toISOString()}: ${msg}`);
    } finally {
        recordSyncFinish();
    }
}
