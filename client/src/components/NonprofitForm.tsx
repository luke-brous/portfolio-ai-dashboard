...
import { useCreateNonprofit, useUpdateNonprofit } from "../hooks/useNonProfits";
...
export default function NonprofitForm({
  ...
  const [grantCycleDates, setCycle] = useState(existing?.grantCycleDates ?? "");
  ...
  // No client-side check for grantCycleDates / grantStatus: both are
  // optional free-form strings, and a blank one is sent as null rather
  // than rejected, so there is nothing for the user to correct here.
  // The server still enforces "non-blank when provided".
  ...
  if (grantAmount.trim() !== "") {
    ...
    else if (parsed <= 0) clientErrors.grantAmount = "Amount must be positive.";
  ...
  return (
    <form
      ...
      aria-label={
        isEdit ? `Edit ${existing?.name ?? "nonprofit"}` : "Add a new nonprofit"
      }
    >
      ...
      {errorKind === "session" && (
        ...
        >
          <p className="font-medium">
            We couldn&rsquo;t authenticate your session. Please log in again to
            save this change.
          </p>
          <a
            href="/auth/login"
